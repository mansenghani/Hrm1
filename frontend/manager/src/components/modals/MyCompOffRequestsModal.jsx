...
const MyCompOffRequestsModal = ({ isOpen, onClose }) => {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const res = await axios.get('/api/comp-off/my', {
        headers: { Authorization: `Bearer ${token}` }
      });
      setRequests(res.data);
    } catch (err) {
      toast.error('Failed to fetch your comp-off requests');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white dark:bg-[#1e293b] rounded-2xl shadow-xl w-full max-w-2xl p-6 relative max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-4">
          {loading && requests.length === 0 ? (
            ...
            <div className="text-center py-4 text-gray-500">You haven't submitted any Comp-Off requests yet.</div>
          ) : (
            requests.map(req => (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-sm font-bold text-gray-900 dark:text-white">
                      Date Worked: {new Date(req.dateWorked).toLocaleDateString()}
                    </span>
                  </div>
...
export default MyCompOffRequestsModal;
