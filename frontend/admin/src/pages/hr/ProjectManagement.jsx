...
const ProjectManagement = () => {
  ...
  const handleRejectAll = async () => {
    ...
    if (pendingProjects.length === 0) {
      alert('No pending project proposals detected in mission control.');
      return;
    }

    if (window.confirm(`⚠️ AUTHORITY ALERT: You are about to mass-reject ${pendingProjects.length} project proposals. This action is final. Proceed?`)) {
      setLoading(true);
      try {
        await Promise.all(
          pendingProjects.map(p => axios.put(`/api/projects/status/${p._id}`, { status: 'rejected' }, { headers }))
        );
        alert(`Institutional Veto Complete: ${pendingProjects.length} proposals successfully declined.`);
        fetchData();
      } catch (err) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <div className="flex gap-4">
          <button
            ...
            className="border border-[#F6465D] text-[#F6465D] hover:bg-[#F6465D] hover:text-white px-8 py-4 rounded-[20px] font-black text-[11px] uppercase tracking-widest shadow-lg transition-all flex items-center gap-3 active:scale-95"
          >
            <Shield size={16} />
            Reject All Pending
          </button>
            ...
            <div className="p-8 flex-1 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center bg-[#F9FAFC] p-3 rounded-2xl border border-[#E6E8EA]">
                  ...
                  <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${project.status === 'active' ? 'bg-[#0ECB81]/10 text-[#0ECB81]' : 'bg-[#F0B90B]/10 text-[#F0B90B]'
                    }`}>
                    {project.status}
                  </span>
                </div>
      ...
      {isModalOpen && createPortal(
        ...
        >
          <div
            ...
            className="w-full max-w-lg bg-white/95 backdrop-blur-3xl rounded-[32px] shadow-[0_40px_120px_rgba(0,0,0,0.15)] border border-white/20 p-8 relative"
          >
            <button onClick={() => setIsModalOpen(false)} className="absolute top-6 right-6 text-slate-400 hover:text-black hover:rotate-90 transition-all">
              <Trash2 size={20} />
            </button>
            <h2 className="text-2xl font-black text-[#1E2026] uppercase tracking-tighter italic mb-6">Forge New <span className="text-[#F0B90B]">Mission</span></h2>
            ...
            <form onSubmit={handleCreateProject} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full p-3 bg-slate-50 rounded-xl border-2 border-transparent focus:border-[#F0B90B] focus:bg-white transition-all outline-none font-bold text-sm"
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Commanding Manager</label>
                <select
                  required
                  className="w-full p-3 bg-slate-50 rounded-xl border-2 border-transparent focus:border-[#F0B90B] focus:bg-white transition-all outline-none font-bold text-sm appearance-none"
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Mission Parameters (Description)</label>
                <textarea
                  required
                  rows="2"
                  placeholder="Define the tactical objectives..."
                  className="w-full p-3 bg-slate-50 rounded-xl border-2 border-transparent focus:border-[#F0B90B] focus:bg-white transition-all outline-none font-bold text-sm resize-none"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Window Start</label>
                  <input
                    required
                    type="date"
                    className="w-full p-3 bg-slate-50 rounded-xl border-2 border-transparent focus:border-[#F0B90B] focus:bg-white transition-all outline-none font-bold text-sm"
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Window End</label>
                  <input
                    type="date"
                    className="w-full p-3 bg-slate-50 rounded-xl border-2 border-transparent focus:border-[#F0B90B] focus:bg-white transition-all outline-none font-bold text-sm"
              ...
              <button type="submit" className="w-full py-4 bg-[#1E2026] text-white rounded-xl font-black text-[12px] uppercase tracking-widest hover:bg-black shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3">
                Initialize Project Protocol <ArrowRight size={18} />
              </button>
            </form>
          </div>
        </div>,
