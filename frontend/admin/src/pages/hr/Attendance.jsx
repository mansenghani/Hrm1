...
const Attendance = () => {
  ...
  const handleRejectAll = async () => {
    const pendingLogs = attendance.filter(a => a.status === 'Pending');
    if (pendingLogs.length === 0) {
      alert('No pending attendance regularization requests detected.');
      return;
    }

    if (window.confirm(`⚠️ AUTHORITY ALERT: You are about to mass-reject ${pendingLogs.length} attendance requests. This action is final. Proceed?`)) {
      setLoading(true);
      try {
        const token = sessionStorage.getItem('token');
        await Promise.all(
          pendingLogs.map(a => axios.put(`/api/attendance/status/${a._id}`, { status: 'Rejected' }, { headers: { Authorization: `Bearer ${token}` } }))
        );
        alert(`Institutional Veto Complete: ${pendingLogs.length} regularization protocols successfully declined.`);
        fetchData();
      } catch (err) {
                ...
                filteredData.map((row, i) => (
                  <tr key={i} className="hover:bg-[#fffdf9] transition-colors group">
                    ...
                    <td className="px-8 py-6 text-right">
                      <span className={`px-4 py-1.5 rounded-[4px] text-[10px] font-bold uppercase tracking-widest ${row.clockOut ? 'bg-[#eceae3] text-[#939084]' : 'bg-[#fffdf9] text-[#24a148] border border-[#24a148]'
                        }`}>
