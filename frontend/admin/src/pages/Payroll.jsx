...
const Payroll = () => {
  ...
  const handleRejectAll = async () => {
    const pendingItems = payroll.filter(p => p.paymentStatus === 'Pending' || !p.paymentStatus);
    if (pendingItems.length === 0) {
      alert('No pending financial traces detected in current ledger.');
      return;
    }

    if (window.confirm(`⚠️ FISCAL AUTHORITY ALERT: You are about to mass-reject ${pendingItems.length} payroll entries. This action will halt all pending disbursements. Proceed?`)) {
      setLoading(true);
      try {
        await Promise.all(
          pendingItems.map(p => axios.put(`/api/payroll/status/${p._id}`, { status: 'Rejected' }, { headers: { Authorization: `Bearer ${token}` } }))
        );
        alert(`Institutional Veto Complete: ${pendingItems.length} fiscal protocols successfully declined.`);
        fetchPayroll();
      } catch (err) {
        console.error('Mass veto failed:', err);
        alert('Institutional Veto disrupted. Check financial node connection.');
      } finally {
        setLoading(false);
      }
    }
