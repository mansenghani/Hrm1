...
const Chat = () => {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 flex bg-white border-t border-l border-[#E6E8EA] overflow-hidden font-sans">
        <div className={`w-full md:w-[350px] lg:w-[400px] flex flex-col border-r border-[#E6E8EA] bg-[#ffffff] relative ${activeChat ? 'hidden md:flex' : 'flex'}`}>
          ...
          <div className={`flex-1 flex flex-col transition-all duration-300 ${isNewChatOpen ? '-translate-x-full opacity-0 invisible absolute inset-0' : 'translate-x-0 opacity-100 visible'}`}>
            <div className="p-3 bg-white flex flex-col gap-3 border-b border-[#E6E8EA]">
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <h2 className="text-[17px] font-bold text-[#1E2026] tracking-tight">Conversations</h2>
                  <svg width="18" height="4" viewBox="0 0 24 6" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-indigo-600 mt-0.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="w-10 h-10 rounded-full flex items-center justify-center border border-gray-100 shadow-sm transition-all bg-white hover:bg-gray-50 text-[#848E9C] hover:text-[#1E2026] cursor-pointer"
                  >
                    <Search size={18} />
                  </button>
                  {/* Filter / More Toggle */}
              ...
              {isSearchBarOpen && (
                <div className="relative animate-in slide-in-from-top-2 duration-200">
                  ...
                  <input
                    ...
                    className="w-full bg-[#F3F4F6] text-[#1E2026] text-sm rounded-full pl-10 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#7F66FF]/20 transition-all border border-gray-100"
                  />
                </div>
              )}
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-[11px] font-semibold transition-all flex items-center gap-1 whitespace-nowrap cursor-pointer border-none ${isActive
                          ? 'bg-gradient-to-r from-violet-600 to-indigo-500 text-white shadow-sm shadow-indigo-200'
                          : 'bg-[#F3F4F6] text-[#848E9C] hover:bg-gray-200 hover:text-[#1E2026]'
                        }`}
                    >
                      {tab.label}
                      {tab.id === 'All' && isActive && <span className="w-1.5 h-1.5 rounded-full bg-white ml-1 inline-block" />}
                      {tab.count !== undefined && tab.count > 0 && (
                        <span className={`px-1.5 py-[1px] rounded-full text-[9px] font-black ${isActive ? 'bg-white text-indigo-600' : 'bg-[#1E2026] text-white'
                          }`}>
                          {tab.count}
                        </span>
                      )}
            ...
            {/* Conversations List */}
            <div className="flex-1 overflow-y-auto bg-white p-3 custom-scrollbar space-y-2">
              {loading ? (
                ...
                displaySidebarItems.map((item) => {
                  ...
                  const isUnread = item.lastMessage && getSenderId(item.lastMessage.senderId) === String(u._id) && item.lastMessage.status !== 'seen';
                  const displayUnreadCount = item.unreadCount > 0 ? item.unreadCount : (isUnread || item.unreadBy?.includes(currentUserId) ? 1 : 0);
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        const target = isSelfItem ? { ...u, isSelf: true } : u;
                        setActiveChat(target);
                        // Mark as seen on every click — covers re-clicking same chat
                        if (!isSelfItem && !item.isGroup && u._id) {
                          markMessagesAsSeen(u._id);
                        }
                      }}
                      className={`flex items-center gap-2.5 p-2 rounded-xl cursor-pointer transition-all border relative overflow-visible ${isSelected
                          ? 'border-indigo-100 bg-[#E8EAFF]/40 shadow-sm shadow-indigo-100/30'
                    ...
                    >
                      ...
                      <div className="relative shrink-0">
                        {item.isGroup ? (
                          (() => {
                            ...
                            return (
                              <div className={`w-9 h-9 rounded-full bg-gradient-to-tr ${grpStyles} flex items-center justify-center font-bold text-sm shadow-sm border`}>
                                <Users size={18} />
                              </div>
                            );
                          })()
                        ) : u.profileImage ? (
                          <img src={getImageUrl(u.profileImage)} alt={displayName} className="w-9 h-9 rounded-full object-cover border border-[#E6E8EA] shadow-sm" />
                        ) : (
                          (() => {
                            const avatarStyles = getAvatarStyles(displayName, false);
                            return (
                              <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm bg-gradient-to-tr ${avatarStyles} shadow-sm border`}>
                      ...
                      <div className="flex-1 min-w-0 overflow-visible relative">
                        <div className="flex justify-between items-center mb-0.5 relative group/chat-item">
                          <div className="flex items-center gap-1.5 min-w-0 pr-6">
                            <h3 className={`text-[13px] font-bold truncate ${isSelected ? 'text-indigo-900 font-extrabold' : 'text-[#1E2026]'}`}>{displayName}</h3>
                            {item.pinnedBy?.includes(currentUserId) && <Pin size={12} className="text-[#848E9C] shrink-0 fill-current" />}
                            {item.mutedBy?.includes(currentUserId) && <BellOff size={12} className="text-[#848E9C] shrink-0" />}
                            {item.lockedBy?.includes(currentUserId) && <Lock size={12} className="text-[#848E9C] shrink-0" />}
                          </div>
                          
                          {/* Time & Hover Chevron */}
                          <div className="flex items-center shrink-0 min-h-[20px] relative">
                            <span className={`text-[10px] font-medium text-[#848E9C] transition-all duration-200 ${openChatMenuId === (item.chatId || u._id) ? 'opacity-0 translate-x-4' : 'group-hover/chat-item:opacity-0 group-hover/chat-item:translate-x-4'}`}>
                            ...
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    setOpenChatMenuId(openChatMenuId === (item.chatId || u._id) ? null : (item.chatId || u._id));
                                }}
                                className={`absolute right-0 text-[#848E9C] hover:text-[#1E2026] transition-all duration-200 ${openChatMenuId === (item.chatId || u._id) ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4 group-hover/chat-item:opacity-100 group-hover/chat-item:translate-x-0'} bg-white hover:bg-gray-100 rounded-full p-1 shadow-sm border border-gray-100 cursor-pointer z-10`}
                            >
                                <ChevronDown size={14} />
                            </button>

                            {openChatMenuId === (item.chatId || u._id) && (() => {
                              const chatId = item.chatId || item._id;
                              const isArchived = item.archivedBy?.includes(currentUserId);
                              const isLocked = item.lockedBy?.includes(currentUserId);
                              const isMuted = item.mutedBy?.includes(currentUserId);
                              const isPinned = item.pinnedBy?.includes(currentUserId);
                              const isUnread = item.unreadBy?.includes(currentUserId);
                              const isBlocked = item.blockedBy?.includes(currentUserId);

                              return (
                              <>
                                <div className="fixed inset-0 z-40" onClick={(e) => { e.stopPropagation(); setOpenChatMenuId(null); }} />
                                <div className="absolute right-0 top-6 bg-white shadow-2xl border border-gray-200 rounded-xl py-2 w-52 z-50 animate-in slide-in-from-top-1 fade-in duration-200 text-left">
                                  <button onClick={(e) => { e.stopPropagation(); setShowContactInfo(true); setOpenChatMenuId(null); }} className="w-full text-left px-4 py-2.5 text-[13px] text-[#1E2026] hover:bg-gray-50 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer">
                                      <Info size={16} className="text-[#848E9C]" /> Contact info
                                  </button>
                                  <button onClick={(e) => { e.stopPropagation(); handleToggleChatState(chatId, 'archive'); setOpenChatMenuId(null); }} className="w-full text-left px-4 py-2.5 text-[13px] text-[#1E2026] hover:bg-gray-50 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer">
                                      <Archive size={16} className="text-[#848E9C]" /> {isArchived ? 'Unarchive chat' : 'Archive chat'}
                                  </button>
                                  <button onClick={(e) => { e.stopPropagation(); handleToggleChatState(chatId, 'lock'); setOpenChatMenuId(null); }} className="w-full text-left px-4 py-2.5 text-[13px] text-[#1E2026] hover:bg-gray-50 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer">
                                      <Lock size={16} className="text-[#848E9C]" /> {isLocked ? 'Unlock chat' : 'Lock chat'}
                                  </button>
                                  <button onClick={(e) => { e.stopPropagation(); handleToggleChatState(chatId, 'mute'); setOpenChatMenuId(null); }} className="w-full text-left px-4 py-2.5 text-[13px] text-[#1E2026] hover:bg-gray-50 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer">
                                      <BellOff size={16} className="text-[#848E9C]" /> {isMuted ? 'Unmute notifications' : 'Mute notifications'}
                                  </button>
                                  <button onClick={(e) => { e.stopPropagation(); handleToggleChatState(chatId, 'pin'); setOpenChatMenuId(null); }} className="w-full text-left px-4 py-2.5 text-[13px] text-[#1E2026] hover:bg-gray-50 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer">
                                      {isPinned ? <PinOff size={16} className="text-[#848E9C]" /> : <Pin size={16} className="text-[#848E9C]" />} {isPinned ? 'Unpin chat' : 'Pin chat'}
                                  </button>
                                  <button onClick={(e) => { e.stopPropagation(); handleToggleChatState(chatId, 'unread'); setOpenChatMenuId(null); }} className="w-full text-left px-4 py-2.5 text-[13px] text-[#1E2026] hover:bg-gray-50 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer">
                                      <CheckCircle2 size={16} className="text-[#848E9C]" /> {isUnread ? 'Mark as read' : 'Mark as unread'}
                                  </button>
                                  <button onClick={(e) => { e.stopPropagation(); setActiveChat(null); setOpenChatMenuId(null); }} className="w-full text-left px-4 py-2.5 text-[13px] text-[#1E2026] hover:bg-gray-50 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer border-b border-gray-100 mb-1 pb-3">
                                      <X size={16} className="text-[#848E9C]" /> Close chat
                                  </button>
                                  <button onClick={(e) => { e.stopPropagation(); handleToggleChatState(chatId, 'block'); setOpenChatMenuId(null); }} className="w-full text-left px-4 py-2.5 text-[13px] text-[#1E2026] hover:bg-gray-50 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer">
                                      <Ban size={16} className="text-[#848E9C]" /> {isBlocked ? 'Unblock contact' : 'Block contact'}
                                  </button>
                                  <button onClick={(e) => { e.stopPropagation(); handleClearSpecificChat(chatId); setOpenChatMenuId(null); }} className="w-full text-left px-4 py-2.5 text-[13px] text-[#1E2026] hover:bg-gray-50 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer">
                                      <MinusCircle size={16} className="text-[#848E9C]" /> Clear chat
                                  </button>
                                  <button onClick={(e) => { e.stopPropagation(); handleDeleteChat(chatId); setOpenChatMenuId(null); }} className="w-full text-left px-4 py-2.5 text-[13px] text-red-500 hover:bg-red-50 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer">
                        ...
                        <div className="flex justify-between items-center gap-2">
                          <p className="text-[11px] truncate text-[#848E9C] flex-1">
                            {isSelfItem ? (
                              item.lastMessage
                                ? <span>{item.lastMessage.message || (item.lastMessage.attachment ? '📎 Attachment' : '')}</span>
                                : <span className="italic text-gray-400">Message yourself</span>
                            ) : item.lastMessage ? (
                              <>
                                  ...
                                  <span className="inline-block mr-1">
                                    {item.lastMessage.status === 'seen' 
                                      ...
                                      : item.lastMessage.status === 'delivered' 
                                        ? <CheckCheck size={14} className="text-[#848E9C] inline" /> 
                                        : <Check size={14} className="text-[#848E9C] inline" />}
                                  </span>
                                )}
            ...
            <div className="p-2 bg-white border-t border-[#E6E8EA] flex justify-center shrink-0">
              <button
                onClick={() => { setSearchQuery(''); setActiveTab('All'); }}
                className="px-4 py-1.5 bg-white border border-gray-100 hover:border-indigo-100 text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50/20 text-[11px] font-bold rounded-full shadow-sm flex items-center gap-1.5 transition-all cursor-pointer"
              >
          ...
          <div className={`flex-1 flex flex-col bg-white transition-all duration-300 absolute inset-0 z-30 ${isNewChatOpen ? 'translate-x-0 opacity-100 visible' : 'translate-x-full opacity-0 invisible'}`}>
            <div className="p-4 bg-[#F5F7FA] border-b border-[#E6E8EA] flex items-center gap-6">
              <button onClick={() => setIsNewChatOpen(false)} className="p-1 hover:bg-gray-200 rounded-full transition-colors border-none bg-transparent cursor-pointer"><ArrowLeft size={20} className="text-[#111B21]" /></button>
              <h2 className="text-lg font-bold text-[#111B21]">New chat</h2>
            </div>
            <div className="p-3 bg-white">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#848E9C]" size={16} />
                <input type="text" value={newChatSearch} onChange={(e) => setNewChatSearch(e.target.value)} placeholder="Search name or ID" className="w-full bg-[#F5F7FA] text-[#1E2026] text-sm rounded-xl pl-10 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#3E74FF]/20 transition-all border border-gray-100" />
              </div>
            </div>
        ...
        <div className={`flex-1 flex flex-row bg-[#F0F2F5] relative ${!activeChat ? 'hidden md:flex items-center justify-center' : 'flex'}`}>
          {!activeChat ? (
            <div className="text-center p-12 bg-white/60 backdrop-blur-md rounded-3xl shadow-xl border border-white/40 max-w-md m-auto">
              <div className="w-24 h-24 bg-gradient-to-tr from-violet-100 to-indigo-100 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner text-indigo-600"><MessageSquare size={40} /></div>
              <h2 className="text-2xl font-black text-[#1E2026] mb-2 tracking-tight">Real-Time Messaging</h2>
              <p className="text-[#848E9C] text-sm max-w-sm mx-auto font-medium">Select a contact to start communicating securely with your team.</p>
            </div>
          ) : (
            <>
              <div className="flex-1 flex flex-col relative min-w-0">
                {/* Top Chat Window Header */}
                <div className="h-[60px] bg-white border-b border-[#E6E8EA] px-4 flex items-center justify-between shrink-0 shadow-sm z-20">
                  <div className="flex items-center gap-2">
                    <button onClick={() => setActiveChat(null)} className="md:hidden mr-1 p-1 hover:bg-gray-100 rounded-full transition-colors border-none bg-transparent cursor-pointer"><ArrowLeft size={18} className="text-[#1E2026]" /></button>
                    <div className="relative shrink-0">
                      {activeChat.isGroup ? (
                        <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-violet-100 to-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-sm"><Users size={18} /></div>
                      ) : (
                        <>
                          {activeChat.profileImage ? (<img src={getImageUrl(activeChat.profileImage)} alt={activeChat.name} className="w-9 h-9 rounded-full object-cover border border-gray-100 shadow-sm" />) : (<div className="w-9 h-9 rounded-full bg-gradient-to-tr from-indigo-50 to-violet-50 text-indigo-600 flex items-center justify-center font-bold text-sm">{(activeChat.name || 'U').charAt(0).toUpperCase()}</div>)}
                          {onlineUsers.has(activeChat._id) && <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-white rounded-full shadow-sm"></div>}
                        </>
                      )}
                    </div>
                    <div>
                      <h3 className="text-[14px] font-extrabold text-[#1E2026] leading-none mb-1">
                        {activeChat.isGroup
                          ? activeChat.groupName
                          : (String(activeChat._id) === String(currentUserId)
                            ? <span className="flex items-center gap-1.5">{currentUserObj?.name || 'Me'} <span className="text-[9px] font-bold bg-[#3E74FF]/10 text-[#3E74FF] px-1 py-[1px] rounded-full">YOU</span></span>
                            : activeChat.name)
                        }
                      </h3>
                      <p className="text-[11px] font-medium flex items-center gap-1.5 leading-none">
                        {activeChat.isGroup ? (
                          <span className="text-[#848E9C]">{activeChat.participants?.length || 0} participants</span>
                        ) : String(activeChat._id) === String(currentUserId) ? (
                          <span className="text-[#848E9C]">Notes to self</span>
                        ) : onlineUsers.has(activeChat._id) ? (
                          <span className="text-emerald-500 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /> Online</span>
                        ) : (
                          <span className="text-[#848E9C]">Offline</span>
                        )}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 relative">
                    ...
                    <button
                      ...
                      className="w-8 h-8 rounded-full bg-white border border-gray-100 flex items-center justify-center shadow-sm text-[#54656F] hover:text-emerald-500 hover:bg-emerald-50 transition-all cursor-pointer"
                      title="Voice Call"
                    >
                      <Phone size={14} />
                    </button>
                    ...
                    <button
                      ...
                      className="w-8 h-8 rounded-full bg-white border border-gray-100 flex items-center justify-center shadow-sm text-[#54656F] hover:text-blue-500 hover:bg-blue-50 transition-all cursor-pointer"
                      title="Video Call"
                    >
                      <Video size={14} />
                    </button>
                    ...
                    <button
                      ...
                      className="w-8 h-8 rounded-full bg-white border border-gray-100 flex items-center justify-center shadow-sm text-[#54656F] hover:text-[#7F66FF] hover:bg-gray-50 transition-all cursor-pointer"
                    >
                      <UserPlus size={14} />
                    </button>
                    {/* More Options button */}
                    <button
                      ...
                      className="w-10 h-10 rounded-full bg-white border border-gray-100 flex items-center justify-center shadow-md text-[#54656F] hover:text-[#7F66FF] hover:bg-gray-50 transition-all cursor-pointer"
                    >
                    ...
                    {showHeaderMenu && (
                      <>
                        ...
                        <div className="absolute right-0 top-12 bg-white border border-[#E6E8EA] shadow-xl rounded-xl py-2 w-48 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-100 flex flex-col">
                          <button
                            onClick={handleClearChat}
                            className="w-full text-left px-4 py-2.5 text-sm text-[#1E2026] hover:bg-[#F5F7FA] font-medium transition-colors border-none bg-transparent cursor-pointer"
                          >
                            Clear Chat
                          </button>
                          <button
                          ...
                          <button
                            ...
                            className="w-full text-left px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 font-medium transition-colors border-none bg-transparent cursor-pointer"
                          >
                            Close Chat
                          </button>
                        </div>
                ...
                {/* Chat Messages Area */}
                <div className="flex-1 overflow-y-auto p-6 space-y-6 relative custom-scrollbar bg-gradient-to-tr from-[#E8EAFF] via-[#F4EBFF] to-[#FFF0F0] overflow-hidden">
                  {/* Blur Spheres */}
                  ...
                  <div className="flex justify-center my-4 relative z-10">
                    <div className="bg-white/80 backdrop-blur-md px-4 py-1.5 rounded-full shadow-sm border border-gray-100 flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-black text-gray-500 uppercase tracking-wider">Today</span>
                    </div>
                  </div>
                    ...
                    return groups.map((group, i) => {
                      ...
                      if (isCallEvent) {
                        return (
                          <div key={msg._id || i} className="flex w-full justify-center relative z-10 py-1">
                            <div className="flex items-center gap-2 rounded-full bg-white/80 border border-white px-4 py-2 text-[12px] font-bold text-[#667085] shadow-sm">
                      ...
                      return (
                        <div key={msg._id || i} id={`msg-${msg._id}`} className={`flex w-full items-start gap-2 relative z-10 ${isMe ? 'justify-end' : 'justify-start'} ${isSelectionMode ? 'cursor-pointer' : ''}`} onClick={() => { if (isSelectionMode) toggleSelectMessage(msg._id); }}>
                          ...
                          {!isMe && <Avatar />}
                          <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[45%]`}>
                            {activeChat?.isGroup && !isMe && (
                              <span className="text-[11px] font-bold text-indigo-600 mb-1 px-1">{profileName}</span>
                            )}
                            <div
                              onContextMenu={(e) => {
                                e.preventDefault();
                                const windowHeight = window.innerHeight;
                                setMenuPosition(e.clientY > windowHeight * 0.6 ? 'top' : 'bottom');
                                setActiveMenuId(activeMenuId === (msg._id || i) ? null : (msg._id || i));
                              }}
                              className={`relative group px-3 py-2 rounded-[16px] shadow-sm transition-all ${msg.isDeleted
                                  ? (isMe ? 'bg-indigo-50/50 text-[#1E2026] italic' : 'bg-gray-100 text-gray-400 italic')
                                  : (isMe
                                    ? 'bg-gradient-to-r from-[#6e54ff] to-[#a25eff] text-white rounded-tr-none'
                                    : 'bg-white/95 text-[#1E2026] border border-white/80 rounded-tl-none'
                                  )
                                ...
                                <>
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => {
                                      ...
                                      const windowHeight = window.innerHeight;
                                      setMenuPosition(rect.top > windowHeight * 0.6 ? 'top' : 'bottom');
                                      setActiveMenuId(activeMenuId === (msg._id || i) ? null : (msg._id || i));
                                    }}
                                  >
                                    <ChevronDown size={16} />
                                  ...
                                  {activeMenuId === (msg._id || i) && (
                                    <>
                                      ...
                                      <div className={`absolute ${menuPosition === 'top' ? 'bottom-0' : 'top-0'} ${isMe ? 'right-0' : 'left-0'} mt-2 bg-white shadow-xl rounded-xl py-2 w-48 z-50 animate-in fade-in zoom-in-95 duration-100 border border-[#E6E8EA] flex flex-col`}>
                                        ...
                                        <div className="flex items-center justify-around px-2 pb-2 border-b border-[#F5F6F6] mb-1 relative">
                                          ...
                                          <button
                                            ...
                                            className="hover:scale-125 transition-transform p-1 text-[#8696A0] bg-gray-50 rounded-full border-none cursor-pointer"
                                          >
                                            <Plus size={18} />
                                          </button>
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); handleReplyMessage(msg); }} className="w-full px-4 py-2 text-left text-[13.5px] hover:bg-[#F5F6F6] flex items-center gap-3 transition-colors text-[#3B4A54] border-none bg-transparent cursor-pointer font-semibold">
                                          <Reply size={16} className="text-[#8696A0]" /> Reply
                                        </button>

                                        {isMe && (
                                          <button onClick={(e) => { e.stopPropagation(); setMessageInfoMsg(msg); setActiveMenuId(null); }} className="w-full px-4 py-2 text-left text-[13.5px] hover:bg-[#F5F6F6] flex items-center gap-3 transition-colors text-[#3B4A54] border-none bg-transparent cursor-pointer font-semibold">
                                            <Info size={16} className="text-[#8696A0]" /> Message info
                                          </button>
                                        )}

                                        <button onClick={(e) => { e.stopPropagation(); handleCopyMessage(msg.message); }} className="w-full px-4 py-2 text-left text-[13.5px] hover:bg-[#F5F6F6] flex items-center gap-3 transition-colors text-[#3B4A54] border-none bg-transparent cursor-pointer font-semibold">
                                          <Copy size={16} className="text-[#8696A0]" /> Copy
                                        </button>

                                        <button onClick={(e) => { e.stopPropagation(); setForwardMsg(msg); setActiveMenuId(null); }} className="w-full px-4 py-2 text-left text-[13.5px] hover:bg-[#F5F6F6] flex items-center gap-3 transition-colors text-[#3B4A54] border-none bg-transparent cursor-pointer font-semibold">
                                          <Forward size={16} className="text-[#8696A0]" /> Forward
                                        </button>

                                        <button onClick={(e) => { e.stopPropagation(); handlePinMessage(msg._id); }} className="w-full px-4 py-2 text-left text-[13.5px] hover:bg-[#F5F6F6] flex items-center gap-3 transition-colors text-[#3B4A54] border-none bg-transparent cursor-pointer font-semibold">
                                          <Pin size={16} className={msg.isPinned ? 'text-[#3E74FF]' : 'text-[#8696A0]'} /> {msg.isPinned ? 'Unpin' : 'Pin'}
                                        </button>

                                        <button onClick={(e) => { e.stopPropagation(); handleStarMessage(msg._id); }} className="w-full px-4 py-2 text-left text-[13.5px] hover:bg-[#F5F6F6] flex items-center gap-3 transition-colors text-[#3B4A54] border-none bg-transparent cursor-pointer font-semibold">
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); setIsSelectionMode(true); toggleSelectMessage(msg._id); setActiveMenuId(null); }} className="w-full px-4 py-2 text-left text-[13.5px] hover:bg-[#F5F6F6] flex items-center gap-3 transition-colors text-[#3B4A54] border-none bg-transparent cursor-pointer font-semibold">
                                          <CheckSquare size={16} className="text-[#8696A0]" /> Select
                                        </button>

                                        <div className="border-t border-[#F5F6F6] mt-1 pt-1">
                                          <button onClick={(e) => { e.stopPropagation(); setDeleteMsgTarget(msg); setShowDeleteModal(true); setActiveMenuId(null); }} className="w-full px-4 py-2 text-left text-[13.5px] hover:bg-red-50 text-red-500 flex items-center gap-3 transition-colors border-none bg-transparent cursor-pointer font-semibold">
                                  ...
                                  {/* REPLY PREVIEW */}
                                  {msg.replyTo && (
                                    <div className={`mb-2 rounded-xl border-l-4 border-indigo-500 p-2 overflow-hidden max-w-full text-left ${isMe ? 'bg-black/10' : 'bg-gray-50'}`}>
                                      <p className={`text-[11px] font-black ${isMe ? 'text-white' : 'text-indigo-600'} truncate`}>
                                        {String(msg.replyTo.senderId?._id || msg.replyTo.senderId) === String(currentUserId) ? 'Me' : (msg.replyTo.senderId?.name || 'User')}
                                      </p>
                                      <p className={`text-[12px] truncate line-clamp-1 italic ${isMe ? 'text-gray-100' : 'text-gray-500'}`}>
                                    ...
                                    msg.attachment && (
                                      <div className="mb-2 rounded-xl overflow-hidden border border-black/5 bg-black/5">
                                        ...
                                        {msg.fileType === 'document' && (
                                          <div className="p-3 flex items-center gap-3 bg-black/5 text-left">
                                            <div className="w-10 h-10 bg-indigo-600 rounded flex items-center justify-center text-white">
                                              <FileText size={20} />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                              <p className={`text-[13px] font-bold truncate ${isMe ? 'text-white' : 'text-gray-800'}`}>{msg.fileName || 'Document'}</p>
                                              <p className={`text-[11px] ${isMe ? 'text-gray-200' : 'text-[#848E9C]'}`}>File</p>
                                            </div>
                              ...
                              {msg.reactions?.length > 0 && (
                                <>
                                  <div
                                    ...
                                    className={`absolute -bottom-3.5 ${isMe ? 'right-4' : 'left-4'} flex items-center gap-1 bg-white shadow-md border border-[#E6E8EA] rounded-full px-2 py-0.5 z-20 hover:scale-110 transition-transform cursor-pointer min-w-[24px] justify-center text-[12px] h-[22px]`}
                                  >
                                    {Object.entries(
                                      ...
                                      return (
                                        <div key={emoji} className="flex items-center gap-0.5 select-none">
                                          <span>{emoji}</span>
                                          <span className={`text-[10px] font-black ${hasReacted ? 'text-[#3E74FF]' : 'text-gray-500'}`}>{count}</span>
                                        </div>
                                      );
                                    })}
                                  </div>
                                    ...
                                    <>
                                      ...
                                      <div className={`absolute bottom-6 ${isMe ? 'right-0' : 'left-0'} z-[70] bg-white border border-[#E6E8EA] shadow-2xl rounded-xl py-2 w-[220px] animate-in fade-in zoom-in-95 duration-200`}>
                                        <div className="px-3 py-1 border-b border-gray-100 flex items-center justify-between">
                                          ...
                                          <button onClick={() => setShowReactionDetailsId(null)} className="p-1 hover:bg-gray-100 rounded-full transition-colors border-none bg-transparent cursor-pointer"><X size={14} /></button>
                                        </div>
                                        <div className="max-h-[150px] overflow-y-auto custom-scrollbar">
                                          {msg.reactions.map((r, idx) => {
                                            ...
                                            return (
                                              ...
                                              >
                                                <div className="flex items-center gap-2">
                                                  <div className="w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center text-[10px] font-bold text-indigo-600">
                ...
                {messages.filter(m => m.isPinned).length > 0 && (
                  <div className="absolute top-[76px] left-0 right-0 z-30 bg-white/95 backdrop-blur-md border-b border-[#E6E8EA] py-1.5 px-4 flex items-center gap-3 shadow-sm animate-in slide-in-from-top-2">
                    ...
                    }} className="text-[11px] font-bold text-[#7F66FF] hover:bg-[#7F66FF]/5 px-3 py-1 rounded-full transition-all border border-indigo-200 shrink-0 bg-transparent cursor-pointer">View</button>
                  </div>
                )}

                {/* Bottom Message Input Bar */}
                <div className="bg-transparent p-4 flex flex-col gap-2 shrink-0 relative z-20">
                  {activeChat.blockedBy?.includes(currentUserId) ? (
                    <div className="bg-white/95 backdrop-blur-md rounded-2xl p-4 flex items-center justify-center shadow-md border border-gray-100 max-w-4xl w-full mx-auto text-center">
                    ...
                    <>
                      {replyingToMessage && (
                    <div className="mx-4 bg-white/95 backdrop-blur-md border-l-4 border-indigo-500 rounded-2xl p-3.5 flex items-center justify-between shadow-lg border border-gray-100 max-w-4xl w-full mx-auto mb-2 animate-in slide-in-from-bottom-2 duration-200">
                      <div className="flex-1 min-w-0 pr-4">
                        <p className="text-[12px] font-black text-indigo-600 mb-0.5 truncate">
                        ...
                        <p className="text-[13px] text-[#848E9C] truncate line-clamp-1 italic">
                          {replyingToMessage.attachment ? (
                            <span className="flex items-center gap-1.5"><ImageIcon size={14} /> Attachment</span>
                          ) : replyingToMessage.message}
                        </p>
                      </div>
                      <button onClick={() => setReplyingToMessage(null)} className="p-1.5 hover:bg-gray-100 rounded-full transition-colors border-none bg-transparent cursor-pointer shrink-0">
                  ...
                  <div className="w-full max-w-4xl mx-auto px-4">
                    {showEmojiPicker && (
                      <div className="absolute bottom-20 left-4 bg-white border border-[#E6E8EA] shadow-2xl rounded-2xl w-80 h-80 p-3 z-[60] flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-200">
                        ...
                        <div className="flex-1 overflow-y-auto grid grid-cols-8 gap-1 custom-scrollbar pr-1">
                          {EMOJI_LIST.map((e, idx) => (<button key={idx} type="button" onClick={() => setNewMessage(prev => prev + e)} className="text-2xl hover:bg-gray-100 rounded-lg flex items-center justify-center p-1.5 transition-all hover:scale-110 active:scale-95 border-none bg-transparent cursor-pointer">{e}</button>))}
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex items-center bg-white/95 backdrop-blur-md rounded-full border border-white/50 shadow-md p-1.5 gap-2 relative z-10 px-3">
                      {/* Plus Button */}
                      <button
                        type="button"
                        onClick={() => setShowAttachmentMenu(!showAttachmentMenu)}
                        className="w-8 h-8 bg-[#6e54ff] hover:bg-[#583eff] text-white rounded-full flex items-center justify-center hover:scale-105 transition-all shrink-0 border-none cursor-pointer"
                      >
                        <Plus size={16} />
                      </button>
                      ...
                      <textarea
                        ...
                        className="flex-1 bg-transparent border-none py-1.5 text-[13px] text-[#1E2026] font-medium focus:outline-none resize-none max-h-32 min-h-[32px] custom-scrollbar leading-tight placeholder-gray-400"
                        rows="1"
                        style={{ height: 'auto' }}
                      />
                      ...
                      <div className="flex items-center gap-0.5 shrink-0">
                        <button type="button" onClick={() => setShowEmojiPicker(!showEmojiPicker)} className={`w-7 h-7 rounded-full flex items-center justify-center transition-all ${showEmojiPicker ? 'text-[#7F66FF]' : 'text-[#848E9C] hover:text-[#1E2026]'} border-none bg-transparent cursor-pointer`}><Smile size={16} /></button>
                        <button type="button" onClick={() => document.getElementById('doc-upload').click()} className="w-7 h-7 rounded-full flex items-center justify-center text-[#848E9C] hover:text-[#1E2026] border-none bg-transparent cursor-pointer"><Paperclip size={16} /></button>
                        <button type="button" onClick={() => document.getElementById('image-upload').click()} className="w-7 h-7 rounded-full flex items-center justify-center text-[#848E9C] hover:text-[#1E2026] border-none bg-transparent cursor-pointer"><ImageIcon size={16} /></button>
                        <button type="button" className="w-7 h-7 rounded-full flex items-center justify-center text-[#848E9C] hover:text-[#1E2026] border-none bg-transparent cursor-pointer"><Mic size={16} /></button>
                      </div>
              ...
              <div className="p-5">
                ...
                <div className="flex flex-col gap-1">
                  <button
                    ...
                    className="w-full text-right py-3 px-4 text-[#3E74FF] hover:bg-gray-50 font-medium transition-colors border-none bg-transparent cursor-pointer"
                  >
                    Delete for me
                  </button>
                  ...
                  <button onClick={() => setShowDeleteModal(false)} className="w-full text-right py-3 px-4 text-[#3E74FF] hover:bg-gray-50 font-medium transition-colors border-none bg-transparent cursor-pointer">Cancel</button>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white w-full max-w-[400px] h-[550px] rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
              <div className="p-4 border-b flex items-center justify-between">
                <div className="flex items-center gap-4 w-full">
                  <button onClick={() => { setForwardMsg(null); setSelectedForwardUsers([]); setForwardSearchQuery(''); }} className="p-2 hover:bg-gray-100 rounded-full transition-colors border-none bg-transparent cursor-pointer"><X size={20} /></button>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
              <div className="p-4 bg-[#F5F7FA] border-b border-[#E6E8EA] flex items-center justify-between">
                ...
                <button onClick={() => setIsCreatingGroup(false)} className="text-[#848E9C] hover:text-[#111B21] transition-colors border-none bg-transparent cursor-pointer"><Search size={20} className="rotate-45" /></button>
              </div>
              ...
              <div className="p-4 bg-[#F5F7FA] border-t border-[#E6E8EA] flex gap-3">
                <button onClick={() => setIsCreatingGroup(false)} className="flex-1 py-3 text-sm font-bold text-[#848E9C] hover:bg-gray-200 rounded-xl transition-colors border-none cursor-pointer">Cancel</button>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
              <div className="p-4 bg-[#F5F7FA] border-b border-[#E6E8EA] flex items-center justify-between">
                ...
                <button onClick={() => setIsNewContactOpen(false)} className="text-[#848E9C] hover:text-[#111B21] transition-colors p-1 hover:bg-gray-200 rounded-full border-none bg-transparent cursor-pointer"><ArrowLeft size={20} /></button>
              </div>
              <div className="p-4">
                <div className="relative mb-2">
                  ...
                  <input
                    ...
                    className="w-full bg-[#F5F7FA] text-[#1E2026] text-sm rounded-xl pl-10 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#3E74FF]/20 transition-all border border-gray-100"
                    autoFocus
                  />
                </div>
              </div>
